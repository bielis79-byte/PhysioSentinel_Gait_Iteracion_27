...
def add_frontal_advanced(seg, fps, scale_cm_per_px=0.0):
    ...
    # v25 · COM 2D ampliado. Sigue siendo un proxy tronco-pelvis, no un COM 3D segmentario.
...
def compute_metrics(df, fps, start_frame, end_frame, view, assistive_device="Sin ayuda", scale_cm_per_px=0.0):
    ...
    if not is_frontal:
        ...
        metrics += [
            ...
            {"key":"com_lateral_excursion_norm_pct","label":"Excursión mediolateral COM 2D normalizada","value":com_norm,"unit":"% anchura pélvica","quality":q if np.isfinite(com_norm) else "No calculable","notes":"v25: P95-P5 del COM proxy, normalizado por la anchura pélvica aparente. Facilita comparación intraindividual; no es COM 3D."},
            {"key":"com_velocity_ml_px_s","label":"Velocidad mediolateral COM 2D (P95)","value":com_vel,"unit":"px/s","quality":q if np.isfinite(com_vel) else "No calculable","notes":"v25: P95 de la velocidad lateral absoluta del COM proxy. Dependiente de la geometría de cámara."},
            {"key":"com_bos_ratio_p95","label":"COM/BOS lateral máximo robusto","value":com_bos_p95,"unit":"ratio","quality":q if np.isfinite(com_bos_p95) else "No calculable","notes":"v25: distancia lateral absoluta del COM proxy respecto al centro entre ambos pies, dividida por media BOS. 0=centro; ~1=centro de un pie."},
            {"key":"com_bos_ratio_mean_abs","label":"COM/BOS lateral medio","value":com_bos_mean,"unit":"ratio","quality":q if np.isfinite(com_bos_mean) else "No calculable","notes":"v25: alejamiento lateral medio del COM proxy respecto al centro de la base estimada."},
...
def _sagittal_joint_symmetry(metrics, prefix, joint, good=3.0, poor=20.0):
    ...
    detail=[]
    if np.isfinite(pdiff): detail.append(f"diferencia P95 D/I {pdiff:.1f}°")
...
def compute_biomechanical_profile(metrics, analysis_mode=None, view=None):
    """IBV-0.2: perfil biomecánico dependiente de la vista.

    Evita mezclar dominios no observables con una cámara concreta y evita penalizar
    rasgos morfológicos por su magnitud absoluta en proyección 2D.
    """
    context=_infer_biomech_context(metrics,analysis_mode,view)
    domains={}; details={}; weights={}; descriptive={}; excluded=[]

    if context == "frontal":
        # 1) Rodilla: SOLO discrepancia bilateral de la proyección. La magnitud absoluta
        # del valgo/varo NO entra en el score (morfología, rotación de cadera, perspectiva).
        kd,_=_metric_value_any(metrics,["frontal_knee_dev_diff"])
        if not np.isfinite(kd):
            kl,_=_metric_value_any(metrics,["dynamic_knee_valgus_l_deg","frontal_knee_dev_l_p95"])
            kr,_=_metric_value_any(metrics,["dynamic_knee_valgus_r_deg","frontal_knee_dev_r_p95"])
            kd=abs(kl-kr) if np.isfinite(kl) and np.isfinite(kr) else np.nan
        domains["Simetría frontal de rodilla D/I"]=_score_lower_better(kd,3.0,15.0) if np.isfinite(kd) else np.nan
        details["Simetría frontal de rodilla D/I"]=(f"diferencia D/I {kd:.1f}°; no se puntúa el valgo absoluto" if np.isfinite(kd) else "Sin datos suficientes")
        weights["Simetría frontal de rodilla D/I"]=0.45

        # 2) Pelvis: se prioriza la diferencia entre lados, no un valor anatómico fijo.
        pl,_=_metric_value_any(metrics,["trendelenburg_drop_l_deg"])
        pr,_=_metric_value_any(metrics,["trendelenburg_drop_r_deg"])
        pdiff=abs(pl-pr) if np.isfinite(pl) and np.isfinite(pr) else np.nan
        domains["Control pélvico bilateral"]=_score_lower_better(pdiff,1.0,10.0) if np.isfinite(pdiff) else np.nan
        details["Control pélvico bilateral"]=(f"diferencia de caída pélvica D/I {pdiff:.1f}°" if np.isfinite(pdiff) else "Sin segmentación bilateral fiable")
        weights["Control pélvico bilateral"]=0.55

        descriptive["Rodilla frontal proyectada"]="Magnitud absoluta mostrada solo como descriptor 2D; no penaliza el índice por sexo/morfología/rotación."
        descriptive["Pie y retropié"]="Descriptivos; excluidos del índice con cámara frontal única por perspectiva, visibilidad distal y ausencia de plano específico del retropié."
        descriptive["COM/BOS y tronco"]="Se muestran para estabilidad y seguimiento, pero no se convierten a puntuación normativa global por falta de umbrales universales transferibles."
        excluded += ["valgo dinámico absoluto", "orientación absoluta del pie", "retropié", "COM/BOS como puntuación", "acoplamiento tronco-pelvis como puntuación"]
        profile_name="Índice biomecánico frontal 2D"
        method_note=("Frontal única: el compuesto resume principalmente SIMETRÍA bilateral de rodilla proyectada y control pélvico. "
                     "No interpreta el valgo absoluto como patología y no usa pie/retropié para penalizar.")

    elif context == "frontal+lateral":
        # Vista lateral aporta cinemática sagital bilateral. Se puntúan diferencias D/I,
        # no desviación frente a un ángulo normativo único.
        for name,joint,w,poor in [
            ("Simetría sagital de rodilla","knee_flex",0.30,20.0),
            ("Simetría sagital de cadera","hip_flex",0.25,20.0),
            ("Simetría sagital de tobillo-pie","ankle_angle",0.20,20.0),
        ]:
            sc,det=_sagittal_joint_symmetry(metrics,"lateral_",joint,3.0,poor)
            domains[name]=sc; details[name]=det; weights[name]=w
        pl,_=_metric_value_any(metrics,["front_trendelenburg_drop_l_deg"])
        pr,_=_metric_value_any(metrics,["front_trendelenburg_drop_r_deg"])
        pdiff=abs(pl-pr) if np.isfinite(pl) and np.isfinite(pr) else np.nan
        domains["Control pélvico frontal bilateral"]=_score_lower_better(pdiff,1.0,10.0) if np.isfinite(pdiff) else np.nan
        details["Control pélvico frontal bilateral"]=(f"diferencia de caída pélvica D/I {pdiff:.1f}°" if np.isfinite(pdiff) else "Sin segmentación frontal bilateral fiable")
        weights["Control pélvico frontal bilateral"]=0.25
        descriptive["Rodilla frontal proyectada"]="Descriptor adicional; el valgo/varo absoluto no penaliza el índice."
        descriptive["Pie y retropié frontal"]="Descriptivos; no entran en el compuesto. La vista lateral contribuye mediante cinemática sagital del tobillo-pie, no mediante pronación/retropié 3D."
        descriptive["Tronco, COM/BOS"]="Descriptores de estrategia/estabilidad; no puntuados sin umbral normativo validado para este proxy 2D."
        excluded += ["valgo dinámico absoluto", "pie/retropié frontal", "COM/BOS como puntuación", "acoplamiento tronco-pelvis como puntuación"]
        profile_name="Índice biomecánico combinado frontal + lateral 2D"
        method_note=("Dos vistas: el compuesto prioriza simetría cinemática sagital D/I de cadera, rodilla y tobillo-pie, más control pélvico frontal. "
                     "La rodilla frontal absoluta y el retropié permanecen descriptivos.")

    else:  # lateral única
        for name,joint,w in [
            ("Simetría sagital de rodilla","knee_flex",0.40),
            ("Simetría sagital de cadera","hip_flex",0.35),
            ("Simetría sagital de tobillo-pie","ankle_angle",0.25),
        ]:
            sc,det=_sagittal_joint_symmetry(metrics,"",joint,3.0,20.0)
            domains[name]=sc; details[name]=det; weights[name]=w
        descriptive["Pelvis/COM frontal"]="No puntuables porque no existe vista frontal/posterior en este registro."
        descriptive["Pie/retropié frontal"]="No puntuable sin vista frontal/posterior específica."
        excluded += ["pelvis frontal", "COM/BOS frontal", "valgo frontal", "retropié frontal"]
        profile_name="Índice biomecánico lateral 2D"
        method_note="Lateral única: el compuesto se limita a simetría sagital bilateral de cadera, rodilla y tobillo-pie."

    available={k:float(v) for k,v in domains.items() if np.isfinite(v)}
    weight_available=sum(weights.get(k,0.0) for k in available)
    total_weight=sum(weights.values()) if weights else 0.0
    score=float(sum(available[k]*weights[k] for k in available)/weight_available) if available and weight_available>0 else np.nan
    coverage=100.0*weight_available/total_weight if total_weight>0 else 0.0

    # Si la cobertura es muy baja, evitamos falsa precisión del compuesto.
    if coverage < 45.0:
        score=np.nan

    # Confianza de medida separada del valor biomecánico.
    tracking_candidates=["tracking_mean","front_tracking_mean","lateral_tracking_mean"]
    straight_candidates=["straight_walking_usable_pct","front_straight_walking_usable_pct","lateral_straight_walking_usable_pct"]
    tracking=_metric_mean_any(metrics,tracking_candidates)
    straight=_metric_mean_any(metrics,straight_candidates)
    qparts=[coverage]
    ...
    return {
        "global_score":score,
        "classification":_biomech_classification(score),
        "domains":domains,
        ...
        "ranked_deficits":ranked,
        "context":context,
        "profile_name":profile_name,
        "method_note":method_note,
        "version":"IBV-0.2 experimental",
    }


def compute_global_gait_index(metrics, analysis_mode=None, view=None):
    """Compatibilidad: desde v27 el antiguo IGPM se reemplaza por IBV-0.2 dependiente de vista."""
...
def compute_locomotor_capacity_index(metrics, functional_support="Independiente", measured_gait_speed=0.0):
    """ICLM-0.1 experimental: capacidad locomotora, separada del perfil biomecánico.
...
def augment_metrics_with_gait_index(metrics, functional_support="Independiente", measured_gait_speed=0.0, analysis_mode=None, view=None):
    """Añade ICLM + perfil biomecánico IBV-0.2 sin duplicar claves derivadas."""
    ...
    rows=[
        {"key":"locomotor_capacity_index","label":"Índice de Capacidad Locomotora","value":loc["score"],"unit":"/100","quality":loc["confidence_label"],"notes":"ICLM-0.1 experimental. Separa capacidad funcional del perfil biomecánico; aplica reglas de déficit dominante."},
        ...
        {"key":"gait_index_global","label":"Perfil biomecánico dependiente de la vista (compatibilidad)","value":idx["global_score"],"unit":"/100","quality":idx["confidence_label"],"notes":"Desde v27 corresponde a IBV-0.2, no al antiguo IGPM-0.1."},
        {"key":"gait_index_coverage_pct","label":"Cobertura perfil biomecánico (compatibilidad)","value":idx["coverage_pct"],"unit":"% peso disponible","quality":idx["confidence_label"],"notes":"Alias histórico del perfil biomecánico IBV-0.2."},
        {"key":"gait_index_confidence","label":"Confianza perfil biomecánico (compatibilidad)","value":idx["confidence_score"],"unit":"/100","quality":idx["confidence_label"],"notes":"Alias histórico del perfil biomecánico IBV-0.2."},
    ]
...
def _annex_interpretation(m):
    ...
    if k.startswith("biomech_profile_index") and finite:
        return _biomech_classification(fv)+" (IBV-0.2 interno; dependiente de la vista y no diagnóstico)."
    if k.startswith("gait_index_global") and finite:
        return _biomech_classification(fv)+" (alias histórico; desde v27 corresponde al perfil biomecánico dependiente de la vista)."
    if q:
    ...
    if st.session_state.get("metrics_done"):
        ...
        if st.session_state.get("mode", "").startswith("2 cámaras"):
            ...
            if chart_front is not None:
                st.markdown("### Cámara 1 · frontal/posterior")
                st.markdown("#### Centro de masa estimado (COM 2D) y estabilidad dinámica")
                ...
                _c4.metric("COM/BOS P95", fmt(metric_value(metrics,"front_com_bos_ratio_p95"),2))
                _ccols=[c for c in ["COM normalizado (% pelvis)","COM respecto BOS (ratio)"] if c in chart_front.columns]
                if _ccols:
                    st.line_chart(chart_front.set_index("time_s")[_ccols], x_label="Tiempo (s)", y_label="COM 2D normalizado")
                st.caption("COM 2D es una estimación tronco-pelvis proyectada; no equivale a un COM segmentario 3D de laboratorio.")
                for title, cols in [
            ...
            if "Frontal" in st.session_state.get("view", ""):
                ...
                cc4.metric("COM/BOS P95", fmt(metric_value(metrics,"com_bos_ratio_p95"),2))
                st.caption("COM 2D = centro corporal estimado a partir de pelvis y tronco. No equivale a un centro de masa segmentario 3D de laboratorio. La normalización por anchura pélvica mejora la comparación intraindividual.")
                _com_cols=[c for c in ["COM normalizado (% pelvis)","COM respecto BOS (ratio)"] if c in chart.columns]
                if _com_cols:
                    st.line_chart(chart.set_index("time_s")[_com_cols], x_label="Tiempo (s)", y_label="COM 2D normalizado")
...
with tabs[9]:
    st.subheader("Capacidad funcional de marcha + Perfil biomecánico dependiente de la vista")
    st.caption(
        "Desde la Versión 27 la capacidad locomotora y el perfil biomecánico se calculan por separado. "
    ...
    if not metrics_idx:
        ...
        st.caption("ICLM-0.1 experimental. La velocidad entra solo si existe una medida válida en m/s; el apoyo externo se registra clínicamente y no se infiere automáticamente del vídeo.")

        st.markdown("### 2. Perfil biomecánico dependiente de la vista")
        score=idx["global_score"]
        b1,b2,b3,b4=st.columns([1.35,1.55,1.0,1.0])
        b1.metric(idx["profile_name"],f"{score:.0f}/100" if np.isfinite(score) else "—")
        b2.metric("Lectura interna",idx["classification"])
        b3.metric("Cobertura",f"{idx['coverage_pct']:.0f}%")
        b4.metric("Confianza",idx["confidence_label"])
        st.info(idx["method_note"])
        if idx["context"]=="frontal":
            st.warning(
                "**Cámara frontal única:** el valgo/varo absoluto proyectado NO penaliza el índice y pie/retropié quedan fuera del compuesto. "
                "La rodilla solo puede contribuir mediante discrepancia D/I, evitando castigar una alineación bilateral ligada a morfología, sexo, rotación de cadera o perspectiva."
            )
        elif idx["context"]=="frontal+lateral":
            st.success(
                "**Frontal + lateral:** el compuesto se basa principalmente en simetría sagital bilateral de cadera, rodilla y tobillo-pie, junto con control pélvico frontal. "
                "Valgo frontal absoluto y pie/retropié frontal siguen siendo descriptores y no penalizan."
            )
        else:
        ...
        with st.expander("Metodología del perfil biomecánico v27"):
            st.write(
                "El IBV-0.2 evita utilizar un único conjunto de pesos para cualquier cámara. En frontal única prioriza comparaciones bilaterales y control pélvico; "
                "en frontal+lateral incorpora simetría sagital de cadera, rodilla y tobillo-pie. Las magnitudes absolutas de valgo frontal, pie/retropié, COM/BOS y acoplamiento tronco-pelvis "
                "se mantienen descriptivas cuando no existe una dirección normativa universal o cuando la geometría 2D puede confundir anatomía con alteración."
            )
...
st.divider()
st.caption("PhysioSentinel Gait Versión 27 · ICLM-0.1 capacidad locomotora · IBV-0.2 dependiente de la vista · valgo absoluto y pie/retropié no penalizadores en frontal · regla de déficit dominante · COM 2D · consistencia paso a paso · anexo completo · exportación")
